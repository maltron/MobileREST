...
${WILDFLY_LOCATION}/bin/jboss-cli.sh --connect --controller=localhost:9990 --command="/subsystem=datasources/data-source=PersonDS:add(driver-name=MySQL-JDBC-DRIVER, jndi-name=java:/jdbc/Person, connection-url=jdbc:mysql://localhost:3306/PERSON_DATA, user-name=${MYSQL_USERNAME}, password=${MYSQL_PASSWORD})"
